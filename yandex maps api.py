...
class Button(Label):
    ...
    def get_event(self, event):
        global lon, lat, flag_of_search, text_out, flag_of_index, flag_flag
        try:
...
def coordinates_of_mouse(mouse_pos, lon, lat):
    ...
    coeffic_x = float(list_of_spn1[index].split(',')[0]) / 600
    coeffic_y = float(list_of_spn1[index].split(',')[1]) / 450
    begin_x = float(lat) - 300 * coeffic_x
    begin_y = float(lon) - 225 * coeffic_y
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            #print(address_mouse)
            text_out = TextBox((305, 0, 285, 50), address_mouse)
